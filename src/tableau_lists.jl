...
"""
tableaus_firk_lobatto() = Tuple(
    ( getfield(GeometricIntegrators, Symbol(:Lobatto, family))(s), "firk_lobatto_$(family)$(s)" )
...
"""
tableaus_vprk_gauss() = _projected(Tuple(
...
"""
tableaus_vprk_lobatto() = _projected(Tuple(
    ( getfield(GeometricIntegrators, Symbol(:VPRKLobatto, family))(s), "vprk_lobatto_$(family)$(s)" )
    for family in LOBATTO_FAMILIES for s in LOBATTO_STAGES
))
...
"""
tableaus_vprk_lobatto_symplectic() = _projected(Tuple(
    ( method(s), "vprk_lobatto_$(name)$(s)" )
    for (name, method) in ((:IIIA_IIIB, VPRKLobattoIIIAIIIB), (:IIIB_IIIA, VPRKLobattoIIIBIIIA))
