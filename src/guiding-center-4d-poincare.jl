...
"""
function odeproblem(kind::Symbol, geometry::Symbol; kwargs...)
    SPECS[kind].ode(GEOMETRIES[geometry]; kwargs...)
end

function iodeproblem(kind::Symbol, geometry::Symbol; kwargs...)
...
function run_poincare(kind::Symbol, geometry::Symbol, problem, name, list,
    ...
    for run in list
        ...
        if isempty(sweep)
            ...
            _save_plot(
                () -> PoincareInvariants.plot_invariant(pinv, sweep...; p = params,
                    title = headline),
                plot_dir, file, "_invariant", fig_suff)
            push!(suffixes, "_invariant")

            # The geometry figures come from the last, finest run of the sweep.
            append!(suffixes, spec.figures(
                last(sweep).second, equ, plot_dir, file, fig_suff))
        end
